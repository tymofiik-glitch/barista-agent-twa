...
def settings_kb(lang: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(t(lang, "btn_back"), callback_data="set_back")],
    ])
...
def usual_view_kb(lang: str, has_usual: bool) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(t(lang, "btn_back"), callback_data="set_back")])
    return InlineKeyboardMarkup(rows)
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if usual:
        usual_str = ", ".join(it.get("product", "") for it in usual)
    else:
        usual_str = t(lang, "settings_no_usual")

    await update.message.reply_text(
        t(lang, "settings_title", name=name, phone=phone, lang=lang_label, usual=usual_str),
...
async def show_usual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st["state"] in ("CART_PENDING", "EDITING") and usual:
        ...
        await update.message.reply_text(t(lang, "usual_added_to_cart"))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── AWAITING_OTHER_TIME — клієнт пише час БЕЗ виклику GPT
    if st["state"] == "AWAITING_OTHER_TIME":
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "edit_time":
        await context.bot.send_message(
            chat_id=chat_id, text=t(lang, "what_time"),
    ...
    if data == "edit_comment":
        ...
        await context.bot.send_message(chat_id=chat_id, text=t(lang, "type_comment"))
        return
    ...
    if data == "set_back":
        try:
            await query.edit_message_text(t(lang, "settings_closed"))
        except Exception:
            pass
        return
    ...
    if data == "set_name":
        ...
        await query.message.reply_text(t(lang, "ask_new_name"))
        return

    if data == "set_lang":
        await query.message.reply_text(t(lang, "ask_lang"), reply_markup=lang_choice_kb())
        return

    if data == "lang_uk":
        save_user_field(user_id, lang="uk")
        await query.edit_message_text(t("uk", "lang_set_uk"))
        users = load_users()
        name = users.get(str(user_id), {}).get("name", "друже").capitalize()
        await context.bot.send_message(
            chat_id=chat_id,
            text=t("uk", "greet_known", name=name),
            reply_markup=main_kb("uk"),
        )
        return

    if data == "lang_en":
        save_user_field(user_id, lang="en")
        await query.edit_message_text(t("en", "lang_set_en"))
        users = load_users()
        name = users.get(str(user_id), {}).get("name", "friend").capitalize()
        await context.bot.send_message(
            chat_id=chat_id,
            text=t("en", "greet_known", name=name),
    ...
    if data == "usual_change":
        st["state"] = "AWAITING_USUAL_SETUP"
        await query.message.reply_text(t(lang, "usual_setup"), parse_mode="Markdown")
        return

    if data == "usual_delete":
        delete_user_field(user_id, "usual_order")
        await query.edit_message_text(t(lang, "usual_deleted"))
        return
